...
func Test_ClaudeComplete_With_EmptyModel_And_CancelledContext_Should_ReturnError(t *testing.T) {
	t.Parallel()
	// Arrange
	c := llm.NewClaudeCompleter("")
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
...
func Test_NewClaudeCompleter_With_EmptyModel_Should_ReturnCompleter(t *testing.T) {
	t.Parallel()
	// Arrange & Act
	c := llm.NewClaudeCompleter("")
